...
<html>
    ...
    ?>
                             ...
                             <div class="form-group">
                                <label>Select Mentor :<a class="tooltip-ps" data-toggle="tooltip" title="Please select Your Mentor"> <span class='glyphicon glyphicon-info-sign menu-icon'></span></a>
                                </label>
                                <select name="mname" class="form-control" required>
                                     <?php

                                $conn=mysqli_connect('localhost','root','','eleavesystem');
                                $que=mysqli_query($conn,"select * from empreg");
                             while ($raws=mysqli_fetch_array($que)) {

                        echo "<option>".$raws["firstname"]."</option>";
                             }
    ?>
</select>
                                <input type="hidden" name="hidden_id" value="<?php echo $row['id']; ?>"/>
                ...
                <hr/>
                <input type="submit" class="btn btn-primary" value="<?php echo($action !='action.php?action=update' ? "Save" : "Update")?>"  id="btn_student" name="btn_student" />               
                <a href="student_history.php" class="btn btn-primary">Cancel</a>
                </div> <!--wrapper div end-->
                    </form>
